...
using static UnityChanDance.VMD.VMDReader.BoneKeyFrameGroup;
namespace UnityChanDance.VMD
{
    public interface IMorphPlayer
    {
        void Morph(int frameNumber);
    }
    public class UnityVMDPlayer : MonoBehaviour
    {
        ...
        //エラー値の再現に用いる
        private readonly Quaternion ZeroQuaternion = new(0, 0, 0, 0);
        ...
        [SerializeField]
        private bool useMorphBlendShape;
        [SerializeField]
        private MorphBlendShape morphBlendShape;
        private void Awake()
        {
            morphBlendShape.BuildMorphMap();
        }
        private void Start()
        {
        ...
        public void Play(VMDReader vmdReader)
        {
            ...
            if (useMorphBlendShape) morphPlayer = new MorphBlendShapePlayer(morphBlendShape, vmdReader);
            else morphPlayer = new MorphPlayer(transform, vmdReader);
        ...
        public void JumpToFrame(int frameNumber)
        {
            ...
            internalFrameNumber = frameNumber;
            AnimateBody(frameNumber);
            morphPlayer?.Morph(frameNumber);
        }
        ...
        {
            Dictionary<BoneNames, (Vector3 localPosition, Quaternion localRotation)> pose = new();
        ...
        void AnimateParentOfAll()
        {
            ...
            parentBoneFrame ??= new VMD.BoneKeyFrame();
        ...
        private void InterpolateParentOfAll()
        {
        ...
        private void EnforceInitialPose(Animator animator, bool aPose = false)
        {
            if (animator == null)
            {
                Debug.Log("EnforceInitialPose");
                Debug.Log("Animatorがnullです");
                return;
            }
        ...
        private class UpperBodyAnimation
        {
            private Quaternion ZeroQuaternion = new(0, 0, 0, 0);
            private readonly Dictionary<BoneNames, (Transform, float)> upperBoneTransformDictionary;
            private readonly Dictionary<BoneNames, Vector3> upperBoneOriginalLocalPositions;
            private readonly Dictionary<BoneNames, Quaternion> upperBoneOriginalLocalRotations;
            private readonly VMDReader vmdReader;
            private readonly BoneGhost boneGhost;
            private readonly Transform LeftUpperArmTwist;
            private readonly Transform RightUpperArmTwist;
        ...
        private class LowerBodyAnimation
        {
            private Quaternion ZeroQuaternion = new(0, 0, 0, 0);
            private readonly Dictionary<BoneNames, (Transform, float)> lowerBoneTransformDictionary;
            private readonly Dictionary<BoneNames, Vector3> lowerBoneOriginalLocalPositions;
            private readonly Dictionary<BoneNames, Quaternion> lowerBoneOriginalLocalRotations;
            private readonly VMDReader vmdReader;
            private readonly BoneGhost boneGhost;
        ...
        private class CenterAnimation
        {
            private readonly Quaternion ZeroQuaternion = new(0, 0, 0, 0);
            private readonly BoneNames centerBoneName = BoneNames.センター;
            private readonly BoneNames grooveBoneName = BoneNames.グルーブ;
        ...
        private class FootIK
        {
            public enum Feet { LeftFoot, RightFoot }

            private const string TargetName = "IKTarget";
            ...
            private readonly Dictionary<Transform, Quaternion> boneOriginalRotationDictionary;
            private readonly Dictionary<Transform, Quaternion> boneOriginalLocalRotationDictionary;

            public Transform Target { get; private set; }
            private readonly BoneNames footBoneName;
        ...
        private class ToeIK
        {
            ...
            private Quaternion footOriginalLocalRotation = Quaternion.identity;

            //全ての親から見た始めの角度
            private Quaternion footOriginalRotation = Quaternion.identity;
            private Quaternion toeIKOriginalRotation = Quaternion.identity;
            ...
            private readonly BoneNames footIKBoneName;
            private readonly BoneNames toeIKBoneName;
            private readonly BoneNames footBoneName;
        ...
        private class BoneGhost
        {
            ...
            private readonly Dictionary<BoneNames, Transform> boneDictionary = new();

            public Transform ParentOfAllGhost;
            private readonly Transform rootGhost;
        ...
        private class MorphPlayer : IMorphPlayer
        {
            VMDReader vmdReader;
            List<SkinnedMeshRenderer> skinnedMeshRendererList;
            //キーはunity上のモーフ名
            readonly Dictionary<string, MorphDriver> morphDrivers = new();
            //vmd上はまばたきというモーフ名でも、unity上では1.まばたきなどありうるので変換
            //unity上のモーフ名でvmd上のモーフ名を含むものを探す
            readonly Dictionary<string, string> unityVMDMorphNameDictionary = new();
            ...
            public MorphPlayer(Transform model, VMDReader vmdReader)
            {
                ...
                static List<SkinnedMeshRenderer> SearchBlendShapeSkins(Transform t)
                {
                    List<SkinnedMeshRenderer> skinnedMeshRendererList = new();
